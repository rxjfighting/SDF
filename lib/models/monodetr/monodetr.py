...
class MonoDETR(nn.Module):
    ...
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
        ...
        self.dim_embed_3d = MLP(hidden_dim, hidden_dim, 4, 2)
    ...
    def forward(self, images, calibs, targets, img_sizes):
        ...
        for lvl in range(hs.shape[0]):
            ...
            size3d = inter_references_dim[lvl, :, :, : 3]
            ...
            #depth_weight
            map_certainty = F.grid_sample(
                torch.max(pred_depth_map_logits, 1).values.unsqueeze(1),
                outputs_center3d,
                mode='bilinear',
                align_corners=True).squeeze(1)
            certainty = F.softmax(torch.cat([torch.exp(-inter_references_dim[lvl, :, :, 3: 4]), depth_reg[:, :, 1: 2], map_certainty], dim=-1), dim=-1)
            reg_weight = certainty[:, :, 1: 2]
            geo_weight = certainty[:, :, 0: 1]
            map_weight = certainty[:, :, 2: 3]

            
            # depth average + sigma
            depth_ave = torch.cat([((1. / (depth_reg[:, :, 0: 1].sigmoid() + 1e-6) - 1.) * reg_weight + depth_geo.unsqueeze(-1) * geo_weight + depth_map * map_weight),
                                    depth_reg[:, :, 1: 2]], -1)
